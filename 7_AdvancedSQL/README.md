<div align="center">

# Advanced SQL

**Course 7 of 17 - advanced querying patterns for analysis on Google BigQuery.**

[![Kaggle](https://img.shields.io/badge/Kaggle-Advanced%20SQL-20BEFF.svg)](https://www.kaggle.com/learn/advanced-sql)
![Status](https://img.shields.io/badge/Status-In%20progress-yellow.svg)
![Lessons](https://img.shields.io/badge/Lessons-1%20of%204-yellow.svg)

</div>

## Course Snapshot

| Field | Detail |
|-------|--------|
| Position | Course 7 of 17 |
| Estimated time | 4 hours |
| Status | In progress |
| Started | May 2026 |
| Course page | [Kaggle Learn: Advanced SQL](https://www.kaggle.com/learn/advanced-sql) |

## Lesson Tracker

| # | Lesson | Exercise |
|:-:|--------|:--------:|
| 1 | JOINs and UNIONs | [Done](./JOINsAndUNIONsExercise.py) |
| 2 | Analytic Functions | Pending |
| 3 | Nested and Repeated Data | Pending |
| 4 | Writing Efficient Queries | Pending |

## Core Skills

- Joining tables with `LEFT JOIN`, `FULL JOIN`, and multi-table joins
- Combining compatible result sets with `UNION DISTINCT`
- Comparing question-and-answer activity across BigQuery public datasets
- Preparing ranked, ordered, and partitioned analysis with analytic functions
- Querying nested and repeated records in BigQuery
- Writing efficient SQL that limits scanned data and keeps analysis reproducible

## Artifacts

- Completed exercise solutions are exported as Python files for quick review.
- The course certificate will be added after all lessons are complete.

<div align="center">

## Progress

**1 / 4 lessons completed**

[Back to Roadmap](../README.md)

</div>
